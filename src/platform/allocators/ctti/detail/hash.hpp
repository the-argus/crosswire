...
/// Compile-time type information for the allo allocator lib
#include <cstdint>
namespace allo::ctti {

/// stuff for hashing strings at compile-time
...
// FNV-1a 64 bit hash
constexpr hash_t fnv1a_hash(std::size_t n, const char *str,
                            hash_t hash = fnv_basis)
{
    return n > 0 ? fnv1a_hash(n - 1, str + 1, (hash ^ *str) * fnv_prime) : hash;
}

template <std::size_t N> constexpr hash_t fnv1a_hash(const char (&array)[N])
{

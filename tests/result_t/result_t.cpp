...
TEST_SUITE("result_t")
{
    TEST_CASE("Construction and type behavior")
    {
        ...
        {
            auto getresiftrue =
                [](bool cond) -> result_t<trivial_t, StatusCodeA> {
                if (cond) {
                    return trivial_t{
                        .whatever = 10,
                        .nothing = nullptr,
            ...
            REQUIRE(getresiftrue(false).status() == StatusCodeA::BadAccess);
        }
        ...
        SUBCASE("construct type directly into result")
        {
            ...
            using res = result_t<constructed_type, StatusCodeA>;
            auto constructed_result = [](bool cond) -> res {
                return cond ? res(std::string("hello"))
                            : res(StatusCodeA::OOMIGuess);
            };
            ...
            REQUIRE(constructed_result(false).status() ==
                    StatusCodeA::OOMIGuess);
        }
    }
    ...
    TEST_CASE("Functionality")
    {
        SUBCASE("formattable")
        {
            using res = result_t<int, StatusCodeB>;
            using refres = result_t<int &, StatusCodeB>;
            res result(10);
            int target = 10;
            refres refresult(target);
        ...
        {
            using res = result_t<trivial_t, StatusCodeB>;
            ...
            REQUIRE(!result.okay());
            REQUIRE(result.status() == StatusCodeB::ResultReleased);
        }
        ...
        SUBCASE("std::vector result")
        {
            ...
            using res = result_t<std::vector<size_t>, VectorCreationStatusCode>;
        ...
        SUBCASE("reference result")
        {
            ...
            using res =
                result_t<std::vector<int> &, ReferenceCreationStatusCode>;
        ...
        SUBCASE("const reference result")
        {
            ...
            using res =
                result_t<const std::vector<int> &, ReferenceCreationStatusCode>;
        ...
        SUBCASE("how much result copies its contents")
        {
            ...
            using res =
                result_t<increment_on_copy_or_move, dummy_status_code_e>;
